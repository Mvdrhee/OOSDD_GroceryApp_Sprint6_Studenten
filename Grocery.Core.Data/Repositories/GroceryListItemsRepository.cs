...
namespace Grocery.Core.Data.Repositories
{
    public class GroceryListItemsRepository : DatabaseConnection, IGroceryListItemsRepository
    {
        ...
        {
            string query = $"INSERT INTO GroceryListItem(GroceryListId, ProductId, Amount) VALUES(@GroceryListId, @ProductId, @Amount)";
            OpenConnection();
            using (SqliteCommand command = new(query, Connection))
            {
        ...
        {
            GroceryListItem? groceryListItem = null;
            string query = $"UPDATE GroceryListItem SET Id, GroceryListId = @GroceryListId, ProductId = @ProductId, Amount = @Amount WHERE Id = {item.Id}";
            OpenConnection();
            using (SqliteCommand command = new(query, Connection))
            {
                command.Parameters.AddWithValue("GroceryListId", item.GroceryListId);
